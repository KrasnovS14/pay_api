...
# Удалить все карты по номеру телефона
...
def get_all_cards_or_exact_transactions(card_id, user_id):
    db = next(get_db())
    if card_id == 0:
        card_monitor = db.query(Transaction).filter_by(user_id=user_id).all()

    else:card_monitor = db.query(Transaction).filter_by(user_id=user_id, card_id=card_id).all()
